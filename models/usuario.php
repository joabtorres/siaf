...
class usuario extends model {
    ...
    public function create($data) {
        $sql = $this->db->prepare('INSERT INTO usuario (cod_instituicao, nome_usuario, sobrenome_usuario, usuario_usuario, email_usuario, senha_usuario, cargo_usuario, genero_usuario, nivel_acesso_usuario, status_usuario, imagem_usuario, data_cadastro_usuario) VALUES(:cod_instituicao, :nome, :sobrenome, :usuario, :email, :senha, :cargo, :sexo, :nivel, :statu, :imagem, :data);');
    ...
    public function update($data) {
        try {
            ...
            return $this->read_specific("SELECT * FROM usuario WHERE cod_usuario=:cod", array('cod' => $data['cod_usuario']));
    ...
    public function newpassword($email) {
        ...
        $result = $this->read_specific('SELECT * FROM usuario WHERE email_usuario=:email', array('email' => $email));
        if (!empty($result)) {
            try {
                $nova_senha = trim($this->password_generato());
                $sql = $this->db->prepare('UPDATE usuario SET senha_usuario = ? WHERE cod_usuario = ? AND email_usuario = ?');
    ...
    public function remove($data) {
        $usuario = $this->read_specific('SELECT * FROM usuario WHERE cod_usuario=:cod', $data);
        if (!empty($usuario)) {
            $sql = $this->db->prepare('DELETE FROM usuario WHERE cod_usuario=:cod');
